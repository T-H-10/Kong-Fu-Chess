...
#     def handle_jump(self, player_color: str, position: str) -> Tuple[bool, str]:
#         """
#         מטפל בקפיצה של שחקן - בודק חוקיות ומבצע את הקפיצה
#         """
#         try:
#             # המרת מיקום אלגברי לתא
#             cell = self.board.algebraic_to_cell(position)
            
#             # בדיקה שיש כלי במיקום
#             if cell not in self.pos_to_piece:
#                 return False, f"אין כלי במיקום {position}"
...
#             return True, f"קפיצה הושלמה בהצלחה: {piece_id} ב-{position}"
            
#         except Exception as e:
#             return False, f"שגיאה בקפיצה: {str(e)}"
